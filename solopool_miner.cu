...
//   10% → ~10% actual utilization
//   50% → ~50% actual utilization
//   80% → ~80% actual utilization
//
// RED ZONE (80-100%): MAXIMUM PERFORMANCE - runs HOT!
//   90% → ~95% actual utilization
//   100% → 100% actual (zero throttling, max hashrate)
//
...
static void LogToGUI(const char *fmt, ...) {
    ...
    if (g_hwndLog) {
        ...
        SendMessage(g_hwndLog, EM_REPLACESEL, FALSE, (LPARAM)buf);
    ...
    if (g_logFile) {
        ...
        fflush(g_logFile);  // Flush immediately so user can see live updates
    }
}
...
static unsigned __stdcall cpu_mining_thread(void *arg) {
    ...
    while (g_miner.running) {
        ...
        // At 10%: 10 * 16 * 0.5 / 100 = 0.8 → 1 thread
        // At 80%: 80 * 16 * 0.5 / 100 = 6.4 → 6 threads
        ...
        if (!g_miner.cpu_enabled || thread_id >= active_threads) {
            // Inactive threads sleep for 1 second to minimize CPU overhead
            Sleep(1000);
            continue;
        }
        ...
        // v1.0.0: CPU delay with RED ZONE support
        int power = g_miner.cpu_power_percent;
        int delay_ms;
        if (power <= REDZONE_THRESHOLD) {
            // Safe zone: interpolate from MIN to 80
            ...
            // RED ZONE: interpolate from 80 to 0
            delay_ms = CPU_DELAY_AT_80 - 
                       ((CPU_DELAY_AT_80 - CPU_DELAY_AT_MAX) * (power - REDZONE_THRESHOLD)) / 
                       (MAX_UTILIZATION_PERCENT - REDZONE_THRESHOLD);
        }
        if (delay_ms > 0) {
            Sleep(delay_ms);
        }
...
static unsigned __stdcall gpu_mining_thread(void *arg) {
    ...
    while (g_miner.running) {
        ...
        // SAFE ZONE (10-80%): Interpolate between MIN and 80 values
        // RED ZONE (80-100%): Interpolate between 80 and MAX values (full throttle!)
        double batch_scale;
        ...
        // v1.0.0: GPU delay with RED ZONE support
        // SAFE ZONE (10-80%): Interpolate between MIN and 80 delays
        // RED ZONE (80-100%): Interpolate between 80 and MAX (0) delays
        int delay_ms;
        if (power <= REDZONE_THRESHOLD) {
            // Safe zone: gradual reduction from MIN to 80
            delay_ms = GPU_DELAY_AT_MIN - 
                       ((GPU_DELAY_AT_MIN - GPU_DELAY_AT_80) * (power - MIN_UTILIZATION_PERCENT)) / 
                       (REDZONE_THRESHOLD - MIN_UTILIZATION_PERCENT);
        } else {
            // RED ZONE: rapid reduction from 80 to 0
            delay_ms = GPU_DELAY_AT_80 - 
                       ((GPU_DELAY_AT_80 - GPU_DELAY_AT_MAX) * (power - REDZONE_THRESHOLD)) / 
                       (MAX_UTILIZATION_PERCENT - REDZONE_THRESHOLD);
        }
        if (delay_ms > 0) {
            Sleep(delay_ms);
...
static void StartMining(void) {
    ...
    g_miner.current_suggested_diff = 2;
    ...
    switch (msg) {
        ...
        case WM_TIMER:
            if (wParam == ID_TIMER_UPDATE) {
                ...
                // Sample utilization and redraw graphs
                sample_utilization();
